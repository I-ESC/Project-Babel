...
class LIMUBertModel4Pretrain(nn.Module):
    ...
    # def forward(self, input_seqs, masked_pos=None):
    #     # input_seqs now has shape [64, 3, seq_len]
    #     input_seqs = input_seqs.transpose(1, 2)  # input_seqs now has shape [64, seq_len, 3]
    #     input_list = torch.split(input_seqs, 1, dim=2)  # Split the last dimension into 3 tensors
    #     h_masked_list = [self.transformer(xi.squeeze(2)) for xi in input_list]  # Process each through the transformer
    #     h_masked = torch.mean(torch.stack(h_masked_list, dim=0), dim=0)  # Take the mean across the 0th dimension
    #     if self.output_embed:
    #         return h_masked
    #     if masked_pos is not None:
    #         masked_pos = masked_pos[:, :, None].expand(-1, -1, h_masked.size(-1))
