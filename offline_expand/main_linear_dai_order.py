...
def get_args_parser():
    ...
    parser.add_argument('--step', type=int, default=3, choices=[1, 2, 3], help='Step size argument')
    parser.add_argument('--version', type=int, default=0, help='The seed value to control the number after mul')
    parser.add_argument('--ood', action='store_true')
    parser.add_argument('--classifier_weight', action='store_true', 
                        help='Include to use classifier weights in the MLP')
    parser.add_argument('--all_modalities', action='store_true', 
                        help='Include to use all modalities for the MLP input')
...
def main(args):
    ...
    # classifier = MLP(input_dim=512, hidden_dim=256, num_classes=args.num_class, num_layers=1, num_features=1)
    # ceshi
    classifier = MLP(input_dim=512*3, hidden_dim=256, num_classes=args.num_class, num_layers=1, num_features=3)
    ...
    val_data =  torch.load(f'val_data{args.version}.pt')
    val_labels = torch.load(f'val_labels{args.version}.pt')
...
def train(train_loader, model, classifier, criterion, optimizer, epoch, args):
    ...
    for i, (images, imus, skel, target) in enumerate(train_loader):
        ...
        # compute output        
        output = model(images, imus, skel)
        # output = classifier(output['imu_embed'])
        # ceshi
        output = classifier(output['imu_embed'], output['image_embed'], output['skeleton_embed'])
...
def objective(trial):
    ...
    # lr = trial.suggest_categorical('lr', [0.1, 0.5, 1])
    # batch_size = trial.suggest_categorical('batch_size', [8, 16, 32])
    # Suggest hyperparameters to be tuned
    weight1 = trial.suggest_uniform('weight1', 0.0, 1.0)
    weight2 = trial.suggest_uniform('weight2', 0.0, 1.0)
    weight3 = trial.suggest_uniform('weight3', 0.0, 1.0)
...
if __name__ == '__main__':
    ...
    # # At the end of the main function, write the best accuracy to a file
    # with open("experiment_results.txt", "a") as file:
    #     experiment_id = f"cw_{args.classifier_weight}_am_{args.all_modalities}"
    #     file.write(f"Experiment {experiment_id}: Best Accuracy = {best_acc1}\n")
    ...
    # with open("experiment_results_imu_video_skel.txt", "a") as file:
