...
def get_args_parser():
    ...
    parser.add_argument('--step', type=int, default=1, choices=[1, 2], help='Step size argument')
...
def main(args):
    ...
    criterion = models_dai.get_loss(args.model, args.ssl_temp, args.ssl_scale, args.step).cuda(args.gpu)
    ...
    # Loading augmented and non-augmented datasets
    augmented_data = torch.load('train_data.pt', weights_only=True)

    # Getting indices for training and validation
    all_indices = list(range(len(train_labels)))
    train_indices, val_indices = train_test_split(
        all_indices, test_size=0.05, stratify=train_labels, random_state=args.version
    )

    # Using indices to select items from augmented and non-augmented datasets
    train_data = [augmented_data[i] for i in train_indices]

    train_dataset = datasets_dai.MyDataset('train', train_data)
    ...
    train_loader = torch.utils.data.DataLoader(
        ...
        sampler=train_sampler, drop_last=True, collate_fn=custom_collate, shuffle=True)
