...
class CLIP(nn.Module):
    ...
    def encode_image(self, images):
        ...
        # # Applying the MLP to all images in the batch directly
        # x = self.mlp_image(images)

        # # Applying the shared MLP and projection layers to the batch
    ...
    def encode_skeleton(self, skeletons):
        # # # print('skeletons', skeletons.shape) # skeletons torch.Size([4, 1, 400])
        # # # import torch.nn.functional as F
        # # # skeletons = F.pad(input=skeletons, pad=(0, 340), mode='constant', value=0)
        # # # skeletons = self.adaptation_layer(skeletons)

        # # Directly apply the MLP to all skeletons in the batch
        # x = self.mlp_skeleton(skeletons)

        # # If your MLP modules support batch processing, you can remove the loop
        # # and apply them directly as below:
