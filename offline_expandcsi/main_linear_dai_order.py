...
def get_args_parser():
    ...
    parser.add_argument('--num_class', type=int, default=6, help='num_class')
...
def main(args):
    ...
    classifier = MLP(input_dim=512*2, hidden_dim=256, num_classes=args.num_class, num_layers=1, num_features=2, weights=[0.3, 0.7]) #nn.Linear(512, args.num_class)
    ...
    # Check the format of labels and encode if necessary
    if isinstance(labels[0], str):  # Assuming labels are strings if the first label is a string
        # Creating a label encoding mapping
        unique_labels = set(labels)
        label_encoding = {label: idx for idx, label in enumerate(unique_labels)}
        print(label_encoding)

        # Apply the encoding to the labels
        encoded_labels = [label_encoding[label] for label in labels]
    else:
        # If labels are already numerical, use them as they are
    ...
    for label in np.unique(encoded_labels):
        label_indices = np.where(np.array(encoded_labels) == label)[0]
        random.shuffle(label_indices)
        train_indices.extend(label_indices[:10])  # Take one sample per class for training
        test_indices.extend(label_indices[10:])  # The remaining samples for testing

    X_train = [segments[idx] for idx in train_indices]
    y_train = [encoded_labels[idx] for idx in train_indices]
    X_test = [segments[idx] for idx in test_indices]
    y_test = [encoded_labels[idx] for idx in test_indices]
    
    def custom_collate(batch):
        data = [item[0][0] for item in batch]  # assuming data is already a tensor
        skel_data = [item[0][1] for item in batch]  # assuming data is already a tensor
        # ceshi
        target = [item[1] for item in batch]
        return data, skel_data, target
    # Replace args.batch_size with your actual batch size
    batch_size = args.batch_size

    train_loader = torch.utils.data.DataLoader(
        list(zip(X_train, y_train)), batch_size=batch_size, drop_last=True, collate_fn=custom_collate, shuffle=True)

    test_loader = torch.utils.data.DataLoader(
        list(zip(X_test, y_test)), batch_size=512, collate_fn=custom_collate, shuffle=False)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # # Suppose 'labels' is your array of labels
...
def train(train_loader, model, classifier, criterion, optimizer, epoch, args):
    ...
    for i, (csi_data, skel_data, target) in enumerate(train_loader):
        # ceshi
        ...
        # output = model(csi_data)
        # output = classifier(output['csi_embed'])
        # ceshi
        output = model(csi_data, skel_data)
        output = classifier(output['csi_embed'], output['skeleton_embed'])

        loss = criterion(output, target)

        # measure accuracy and record loss
        acc1, acc5 = accuracy(output, target, topk=(1, 5))
        losses.update(loss.item(), len(csi_data))
        top1.update(acc1.item(), len(csi_data))
        top5.update(acc5.item(), len(csi_data))
...
def validate(val_loader, model, classifier, criterion, args, final=True):
    ...
    with torch.no_grad():
        ...
        for i, (csi_data, skel_data, target) in enumerate(val_loader):
            # ceshi
            target = torch.tensor(target).cuda()
    ...
    if final:
        ...
        plot_confusion_matrix(confusion_matrix, range(6))
...
if __name__ == '__main__':
    ...
    with open("experiment_results_wifi_skel.txt", "a") as file:
