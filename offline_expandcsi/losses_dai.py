...
class CLIPLoss(nn.Module):
    ...
    def forward(self, outputs, optimizer, model):
        ...
        csi_embed_all, skel_embed_all = \
            utils.all_gather_batch([csi_embed, skel_embed])

        # cosine similarity as logits
        logits_per_csi = logit_scale * csi_embed @ skel_embed_all.t()
        logits_per_skel = logit_scale * skel_embed @ csi_embed_all.t()
        ...
        weight_csi, weight_skel = 1.0, 1.0
        ...
        # print(weight_csi, weight_skel)

        # Normalize weights
        total_weight = weight_csi + weight_skel
        weight_csi /= total_weight
        weight_skel /= total_weight

        # Combine losses with new weights and perform final backward pass
        combined_loss = weight_csi * loss_csi + weight_skel * loss_skel
